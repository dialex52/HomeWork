# 5. Подумайте, как можно сделать оптимизацию кода по памяти, по скорости.
# Представлен список чисел. Определить элементы списка, не имеющие повторений. Сформировать из этих элементов список
# с сохранением порядка их следования в исходном списке, например:
# src = [2, 2, 2, 7, 23, 1, 44, 44, 3, 2, 10, 7, 4, 11]
# result = [23, 1, 3, 10, 4, 11]
# Подсказка: напишите сначала решение «в лоб». Потом подумайте об оптимизации.

src = [2, 2, 2, 7, 23, 1, 44, 44, 3, 2, 10, 7, 4, 11]
# решение "в лоб"
result = []
for number in src:
    if src.count(number) == 1:
        result.append(number)
print(result)

# решение с помошью генератора списков

result = [number for number in src if src.count(number) == 1]
print(result)

# Пытался измерить скорость выполнения первой и второй программы функцией time, все очень быстро,
# не улавливает.
# Памяти, я так понимаю, и в первом и во втором случае расходуется одинаково N*N, т.к. count пробегается каждый раз
# в цикле по всем элементам.